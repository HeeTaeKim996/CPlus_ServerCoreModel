...
public:
    template<typename T>
    inline void BindParam(int32 idx, T& value)
    {
        _dbConnection.BindParam(idx + 1, &value, &_paramIndex[idx]);
        _paramFlag |= (1LL << idx);
    }

    inline void BindParam(int32 idx, const WCHAR* value)
    {
        _dbConnection.BindParam(idx + 1, value, &_paramIndex[idx]);
        _paramFlag |= (1LL << idx);
    }
    ...
    inline void BindParam(int32 idx, T (&value)[N])
    {
    ...
    template<typename T>
    inline void BindParam(int32 idx, T* value, int32 N)
    {
        _dbConnection.BindParam(idx + 1, (const BYTE*)value, size32(T) * N, &_paramIndex[idx]);
        _paramFlag |= (1LL << idx);
    }
    ...
    template<typename T>
    inline void BindCol(int32 idx, T& value)
    {
    ...
    template<int32 N>
    inline void BindCol(int32 idx, WCHAR(&value)[N])
    {
        _dbConnection.BindCol(idx + 1, value, N - 1, &_columnIndex[idx]);
        _columnFlag |= (1LL << idx);
    }

    inline void BindCol(int32 idx, WCHAR* value, int32 len)
    {
    ...
    template<typename T, int32 N>
    inline void BindCol(int32 idx, T(&value)[N])
    {
